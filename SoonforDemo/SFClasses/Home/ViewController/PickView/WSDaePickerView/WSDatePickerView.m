...
@interface WSDatePickerView ()<UIPickerViewDelegate,UIPickerViewDataSource,UIGestureRecognizerDelegate> {
    ...
    IBOutlet UILabel *verticalLine;
    IBOutlet UILabel *horLine;
    IBOutlet UIView *topView;
    IBOutlet UILabel *topTimeLib;
    IBOutlet UILabel *topHorLine;
...
-(void)setupUI {
    self.buttomView.layer.cornerRadius = 2;
    self.buttomView.layer.masksToBounds = YES;
    //self.doneButtonColor = RGB(247, 133, 51);
    self.frame=CGRectMake(0, 0, SFSCREENWIDTH, SFSCREENHEIGHT);
    ...
    self.backgroundColor = SFRGBA_Color(0, 0, 0, 0);
    
    [[APPDELEGATE window] bringSubviewToFront:self];
    

    CGFloat bottomBtnHeigth = 44;
    WeakSelf(self);
    ...
    [self.showYearView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.right.top.equalTo(weakself.buttomView);
        make.bottom.equalTo(weakself.buttomView).mas_offset(-bottomBtnHeigth);
    }];
    
    [horLine mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.right.equalTo(weakself.buttomView);
        make.height.mas_equalTo(1);
        make.bottom.mas_equalTo(weakself.buttomView).mas_offset(-bottomBtnHeigth);
    }];
    
    [cancelBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(weakself.buttomView);
        make.bottom.equalTo(weakself.buttomView);
        make.width.equalTo(weakself.buttomView).multipliedBy(0.5);
        make.height.mas_equalTo(bottomBtnHeigth);
    }];
    
    [self.doneBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.right.bottom.equalTo(weakself.buttomView);
        make.width.equalTo(weakself.buttomView).multipliedBy(0.5);
        make.bottom.equalTo(weakself.buttomView);
        make.height.mas_equalTo(bottomBtnHeigth);
    }];
    ...
    [verticalLine mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerX.equalTo(weakself.buttomView);
    ...
    [topView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.bottom.equalTo(weakself.buttomView.mas_top);
        make.width.left.equalTo(weakself.buttomView);
        make.height.mas_equalTo(44);
    }];
    
    [topTimeLib mas_makeConstraints:^(MASConstraintMaker *make) {
        make.edges.equalTo(self->topView);
    }];
    
    [topHorLine mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.right.equalTo(self->topView);
        make.height.mas_equalTo(1);
        make.bottom.equalTo(self->topView).mas_offset(-1);
    }];
...
-(void)defaultConfig {
    ...
    topTimeLib.text = [_scrollToDate stringWithFormat:[_dateFormatter stringByReplacingOccurrencesOfString:@"-" withString:@"/"]];
...
-(void)addLabelWithName:(NSArray *)nameArr {
    ...
    if (!_dateLabelColor) {
        _dateLabelColor =  SFRGB_Color(247, 133, 51);
    }
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component
{
    ...
    topTimeLib.text = [self.scrollToDate stringWithFormat:_dateFormatter];
...
//        self.bottomConstraint.constant = bottom_height;
        self.backgroundColor = SFRGBA_Color(0, 0, 0, 0.4);
        [self layoutIfNeeded];
    }];
}
-(void)dismiss {
